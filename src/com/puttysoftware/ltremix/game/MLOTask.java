...
final class MLOTask extends Thread {
    ...
    private AbstractArenaObject[] doMovementOnce() {
        ...
        try {
            ...
            try {
                uoo = m.getCell(px + this.sx, py + this.sy, pz,
    ...
    private boolean checkLoopCondition(final boolean zproceed,
            final AbstractArenaObject lgo, final AbstractArenaObject ugo) {
    ...
    static boolean checkSolid(final int zx, final int zy) {
        final GameManager gm = LTRemix.getApplication().getGameManager();
        final AbstractArenaObject next = LTRemix.getApplication()
