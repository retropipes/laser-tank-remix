...
public final class CurrentArenaData extends AbstractArenaData {
    ...
    private LowLevelNumberDataStore startData;
    private LowLevelNumberDataStore savedStartData;
    ...
    public CurrentArenaData() {
        this.pastEraData = new LowLevelArenaDataStore(
                AbstractArenaData.MIN_COLUMNS, AbstractArenaData.MIN_ROWS,
                AbstractArenaData.MIN_FLOORS, ArenaConstants.NUM_LAYERS);
        this.presentEraData = new LowLevelArenaDataStore(
                AbstractArenaData.MIN_COLUMNS, AbstractArenaData.MIN_ROWS,
                AbstractArenaData.MIN_FLOORS, ArenaConstants.NUM_LAYERS);
        this.futureEraData = new LowLevelArenaDataStore(
                AbstractArenaData.MIN_COLUMNS, AbstractArenaData.MIN_ROWS,
                AbstractArenaData.MIN_FLOORS, ArenaConstants.NUM_LAYERS);
        ...
        this.dirtyData = new LowLevelFlagDataStore(
                AbstractArenaData.MIN_COLUMNS, AbstractArenaData.MIN_ROWS,
                AbstractArenaData.MIN_FLOORS);
        this.pastSavedTowerState = new LowLevelArenaDataStore(
                AbstractArenaData.MIN_ROWS, AbstractArenaData.MIN_COLUMNS,
                AbstractArenaData.MIN_FLOORS, ArenaConstants.NUM_LAYERS);
        this.presentSavedTowerState = new LowLevelArenaDataStore(
                AbstractArenaData.MIN_ROWS, AbstractArenaData.MIN_COLUMNS,
                AbstractArenaData.MIN_FLOORS, ArenaConstants.NUM_LAYERS);
        this.futureSavedTowerState = new LowLevelArenaDataStore(
                AbstractArenaData.MIN_ROWS, AbstractArenaData.MIN_COLUMNS,
                AbstractArenaData.MIN_FLOORS, ArenaConstants.NUM_LAYERS);
        this.startData = new LowLevelNumberDataStore(
                CurrentArenaData.PLAYER_DIMS, CurrentArenaData.NUM_PLAYERS,
                CurrentArenaData.NUM_ERAS);
        this.startData.fill(-1);
        this.savedStartData = new LowLevelNumberDataStore(
                CurrentArenaData.PLAYER_DIMS, CurrentArenaData.NUM_PLAYERS,
                CurrentArenaData.NUM_ERAS);
    ...
    public CurrentArenaData clone() {
        try {
            ...
            copy.pastEraData = (LowLevelArenaDataStore) this.pastEraData
                    .clone();
            copy.presentEraData = (LowLevelArenaDataStore) this.presentEraData
                    .clone();
            copy.futureEraData = (LowLevelArenaDataStore) this.futureEraData
                    .clone();
            copy.pastSavedTowerState = (LowLevelArenaDataStore) this.pastSavedTowerState
                    .clone();
            copy.presentSavedTowerState = (LowLevelArenaDataStore) this.presentSavedTowerState
                    .clone();
            copy.futureSavedTowerState = (LowLevelArenaDataStore) this.futureSavedTowerState
                    .clone();
            copy.startData = (LowLevelNumberDataStore) this.startData.clone();
            copy.savedStartData = (LowLevelNumberDataStore) this.savedStartData
                    .clone();
    ...
    @Override
    public AbstractArenaObject getCellEra(final int row, final int col,
    ...
    public int getStartRow() {
        return this.startData.getCell(1, CurrentArenaData.PLAYER_1,
                this.activeEra);
    }

    @Override
    public int getStartColumn() {
        return this.startData.getCell(0, CurrentArenaData.PLAYER_1,
                this.activeEra);
    }

    @Override
    public int getStartFloor() {
        return this.startData.getCell(2, CurrentArenaData.PLAYER_1,
                this.activeEra);
    }
    ...
    public boolean doesPlayerExist() {
        ...
        return px0 != -1 && py0 != -1 && pz0 != -1 && px1 != -1 && py1 != -1
                && pz1 != -1 && px2 != -1 && py2 != -1 && pz2 != -1;
    }
            ...
            for (x = 0; x < this.getColumns(); x++) {
                for (y = 0; y < this.getRows(); y++) {
                    for (z = 0; z < this.getFloors(); z++) {
                        final AbstractArenaObject mo = this.getCell(y, x, z,
                                t.getPrimaryLayer());
                        if (mo != null) {
            ...
            for (x = 0; x < this.getColumns(); x++) {
                for (y = 0; y < this.getRows(); y++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        final AbstractArenaObject mo = this.getCell(y, x,
                                floorFix, w);
                        if (mo != null) {
    ...
    @Override
    public void checkForEnemies(final int floorIn, final int enemyLocXIn,
            final int enemyLocYIn, final AbstractArenaObject enemy) {
        ...
        final boolean scanE = this.linearScan(enemyLocX, enemyLocY, floor,
                DirectionConstants.EAST);
        if (scanE) {
            try {
                final AntiTank at = (AntiTank) this.getCell(this.foundX,
                        this.foundY, floor, template.getPrimaryLayer());
                at.kill(this.foundX, this.foundY);
            } catch (final ClassCastException cce) {
                // Ignore
            }
        }
        final boolean scanW = this.linearScan(enemyLocX, enemyLocY, floor,
        ...
        final boolean scanS = this.linearScan(enemyLocX, enemyLocY, floor,
        ...
        final boolean scanN = this.linearScan(enemyLocX, enemyLocY, floor,
    ...
    public int checkForMagnetic(final int floor, final int centerX,
            final int centerY, final int dir) {
        if (dir == DirectionConstants.EAST) {
            return this.linearScanMagnetic(centerX, centerY, floor,
                    DirectionConstants.EAST);
    ...
    @Override
    public boolean linearScan(final int xIn, final int yIn, final int zIn,
            final int d) {
        // Perform the scan
        int xFix = xIn;
        int yFix = yIn;
        ...
        if (d == DirectionConstants.NORTH) {
            final AbstractArenaObject tank = LTRemix.getApplication()
            ...
            if (tank.getSavedObject().isSolid()) {
                ...
                for (u = xFix - 1; u >= 0; u--) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        try {
                            final AbstractArenaObject obj = this.getCell(u,
                                    yFix, zFix, w);
                            if (obj.isOfType(TypeConstants.TYPE_ANTI)) {
                                final int[] unres = DirectionResolver
                                        .unresolveRelativeDirection(
    ...
    @Override
    public int linearScanMagnetic(final int xIn, final int yIn, final int zIn,
            final int d) {
        // Perform the scan
        int xFix = xIn;
        int yFix = yIn;
        ...
        if (d == DirectionConstants.NORTH) {
            for (u = yFix - 1; u >= 0; u--) {
                for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                    try {
                        final AbstractArenaObject obj = this.getCell(xFix, u,
                                zFix, w);
            ...
            for (u = xFix - 1; u >= 0; u--) {
                for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                    try {
                        final AbstractArenaObject obj = this.getCell(u, yFix,
                                zFix, w);
                        if (obj.getMaterial() == MaterialConstants.MATERIAL_MAGNETIC) {
                            return xFix - u - 1;
                        }
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                for (int w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                    try {
                        final AbstractArenaObject obj = this.getCell(x, y, z,
                                w);
    ...
    @Override
    public int[] tunnelScan(final int xIn, final int yIn, final int zIn,
    ...
    @Override
    public void circularScanRange(final int xIn, final int yIn, final int zIn,
        ...
        for (u = xFix - r; u <= xFix + r; u++) {
            for (v = yFix - r; v <= yFix + r; v++) {
                ...
                for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                    try {
                        this.getCell(u, v, zFix, w).rangeAction(xFix, yFix,
                                zFix, u - xFix, v - yFix, rangeType,
                                forceUnits);
                    } catch (final ArrayIndexOutOfBoundsException aioob) {
                        // Do nothing
                    }
    ...
    public boolean circularScanTank(final int x, final int y, final int z,
            final int r) {
        final int[] tankLoc = LTRemix.getApplication().getGameManager()
                .getTankLocation();
        int fX = x;
        int fY = y;
        int fZ = z;
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                for (int z = 0; z < this.getFloors(); z++) {
                    final AbstractArenaObject obj = this.getCell(y, x, z,
    ...
    @Override
    public void fullScanMoveObjects(final int locZ, final int dirX,
            final int dirY) {
        // Perform the scan
        final Application app = LTRemix.getApplication();
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                final AbstractArenaObject obj = this.getCell(y, x, locZ,
                ...
                if (obj instanceof AbstractMovableObject) {
                    ...
                    if (mobj.didPreCheck() && !mobj.didMove()) {
                        app.getGameManager().updatePushedPosition(x, y,
                                x + dirX, y + dirY, mobj);
                        mobj.setMoved();
                    }
                }
            ...
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                for (int z = 0; z < this.getFloors(); z++) {
                    ...
                    if (obj instanceof AntiTank) {
                        // Kill the tank
                        final GameManager gm = LTRemix.getApplication()
                                .getGameManager();
            ...
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                for (int z = 0; z < this.getFloors(); z++) {
                    ...
                    if (!(obj instanceof Ground)) {
                        // Freeze the ground
                        LTRemix.getApplication().getGameManager()
    ...
    @Override
    public void fullScanAllButtonOpen(final int zIn,
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            ...
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                ...
                final AbstractArenaObject obj = this.getCell(y, x, zFix,
                        source.getPrimaryLayer());
        ...
        if (flag) {
            ...
            if (!(this.getCell(dx, dy, zFix,
                    source.getPrimaryLayer()) instanceof Ground)) {
                this.setCell(new Ground(), dx, dy, zFix,
                        source.getPrimaryLayer());
    ...
    @Override
    public void fullScanAllButtonClose(final int zIn,
            final AbstractButton source) {
        // Perform the scan
        int zFix = zIn;
        if (this.thirdDimensionWraparoundEnabled) {
        ...
        if (flag) {
            ...
            if (!this.getCell(dx, dy, zFix, source.getPrimaryLayer()).getClass()
                ...
                this.setCell(source.getButtonDoor(), dx, dy, zFix,
                        source.getPrimaryLayer());
    ...
    @Override
    public void fullScanButtonBind(final int dx, final int dy, final int zIn,
    ...
    @Override
    public void fullScanButtonCleanup(final int px, final int py, final int zIn,
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                ...
                final AbstractArenaObject obj = this.getCell(x, y, zFix,
                        button.getPrimaryLayer());
                if (obj instanceof AbstractButton) {
                    if (((AbstractButton) obj).boundButtonDoorEquals(button)) {
                        this.setCell(new Ground(), x, y, zFix,
                                button.getPrimaryLayer());
                    }
                }
            }
    ...
    @Override
    public void fullScanFindButtonLostDoor(final int zIn,
    ...
    @Override
    public void setCellEra(final AbstractArenaObject mo, final int row,
            ...
            for (int col = 0; col < this.getColumns(); col++) {
                for (int floor = 0; floor < this.getFloors(); floor++) {
                    for (int layer = 0; layer < ArenaConstants.NUM_VIRTUAL_LAYERS; layer++) {
                        this.setVirtualCell(new Empty(), row, col, floor,
                                layer);
                    }
                }
            }
    ...
    @Override
    public void setPlayerRow(final int newPlayerRow, final int playerNum,
            final int era) {
        int fR = newPlayerRow;
        if (this.horizontalWraparoundEnabled) {
            fR = this.normalizeRow(fR);
    ...
    @Override
    public void setPlayerColumn(final int newPlayerColumn, final int playerNum,
            final int era) {
    ...
    @Override
    public void setPlayerFloor(final int newPlayerFloor, final int playerNum,
            final int era) {
    ...
    public void save() {
        ...
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        final AbstractArenaObject oldObj = this.getCellEra(y, x,
                                z, w, EraConstants.ERA_PAST);
    ...
    public void restore() {
        ...
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        final AbstractArenaObject oldObj = (AbstractArenaObject) this.pastSavedTowerState
                                .getCell(x, y, z, w);
                        final AbstractArenaObject newObj = oldObj.clone();
                        this.setCellEra(newObj, y, x, z, w,
                                EraConstants.ERA_PAST);
                    }
                }
            }
        }
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        final AbstractArenaObject oldObj = (AbstractArenaObject) this.presentSavedTowerState
                                .getCell(x, y, z, w);
                        final AbstractArenaObject newObj = oldObj.clone();
                        this.setCellEra(newObj, y, x, z, w,
                                EraConstants.ERA_PRESENT);
                    }
                }
            }
        }
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        final AbstractArenaObject oldObj = (AbstractArenaObject) this.futureSavedTowerState
                                .getCell(x, y, z, w);
                        final AbstractArenaObject newObj = oldObj.clone();
                        this.setCellEra(newObj, y, x, z, w,
                                EraConstants.ERA_FUTURE);
                    }
                }
            }
        }
    ...
    public void saveStart() {
        this.savedStartData = (LowLevelNumberDataStore) this.startData.clone();
    }

    @Override
    public void restoreStart() {
        this.startData = (LowLevelNumberDataStore) this.savedStartData.clone();
    }
            ...
            for (v = 0; v < x; v++) {
                for (w = 0; w < z; w++) {
                    for (t = 0; t < ArenaConstants.NUM_LAYERS; t++) {
                        try {
                            tempStorage.setCell(this.getCell(v, u, w, t), u, v,
                                    w, t);
                        } catch (final ArrayIndexOutOfBoundsException aioob) {
                            // Do nothing
                        }
        ...
        for (u = 0; u < y; u++) {
            for (v = 0; v < x; v++) {
                for (w = 0; w < z; w++) {
                    for (t = 0; t < ArenaConstants.NUM_LAYERS; t++) {
                        try {
                            tempStorage.setCell(this.presentSavedTowerState
                                    .getCell(v, u, w, t), u, v, w, t);
                        } catch (final ArrayIndexOutOfBoundsException aioob) {
                            // Do nothing
                        }
                    }
    ...
    @Override
    public void fillNulls(final AbstractArenaObject fill1,
                ...
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        if (this.getCell(y, x, z, w) == null) {
                            if (w == ArenaConstants.LAYER_LOWER_GROUND) {
                                this.setCellEra(fill1, y, x, z, w,
                                        this.activeEra);
                            } else if (w == ArenaConstants.LAYER_LOWER_OBJECTS
                                    && was16) {
                                if (x >= 16 || y >= 16) {
                                    this.setCellEra(fill2, y, x, z, w,
                                            this.activeEra);
                                } else {
                                    this.setCellEra(new Empty(), y, x, z, w,
                                            this.activeEra);
                                }
                            } else {
                                this.setCellEra(new Empty(), y, x, z, w,
    ...
    public void fillSTSNulls(final AbstractArenaObject fill) {
        ...
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        if (this.presentSavedTowerState.getCell(y, x, z,
                                w) == null) {
                            if (w == ArenaConstants.LAYER_LOWER_GROUND) {
                                this.presentSavedTowerState.setCell(fill, y, x,
                                        z, w);
                            } else {
                                this.presentSavedTowerState.setCell(new Empty(),
                                        y, x, z, w);
                            }
                        }
                    }
    ...
    @Override
    public void writeData(final XDataWriter writer, final ProgressTracker pt)
            throws IOException {
        int y, x, z, w, e;
    ...
    private static CurrentArenaData readTowerG1(final XDataReader reader,
            final int ver, final ProgressTracker pt) throws IOException {
        ...
        for (x = 0; x < arenaSizeX; x++) {
            for (y = 0; y < arenaSizeY; y++) {
                for (z = 0; z < arenaSizeZ; z++) {
                    final AbstractArenaObject obj = LTRemix.getApplication()
                            .getObjects().readArenaObjectG2(reader, ver);
                    lt.setCellEra(obj, y, x, z, obj.getPrimaryLayer(),
                            EraConstants.ERA_PRESENT);
                }
            }
        }
        ...
        for (y = 0; y < CurrentArenaData.PLAYER_DIMS; y++) {
            lt.startData.setCell(reader.readInt(), y, CurrentArenaData.PLAYER_1,
                    EraConstants.ERA_PRESENT);
        }
        if (pt != null) {
            pt.updateProgress();
        ...
        try {
            ...
            lt.futureEraData = (LowLevelArenaDataStore) lt.presentEraData
                    .clone();
        } catch (final CloneNotSupportedException cnse) {
            LTRemix.logError(cnse);
        }
        for (x = 0; x < CurrentArenaData.PLAYER_DIMS; x++) {
            for (y = 0; y < CurrentArenaData.NUM_PLAYERS; y++) {
                for (z = 0; z < CurrentArenaData.NUM_ERAS; z++) {
                    final int curr = lt.startData.getCell(x, y,
                            EraConstants.ERA_PRESENT);
    ...
    private static CurrentArenaData readTowerG2(final XDataReader reader,
            final int ver, final ProgressTracker pt) throws IOException {
        int y, x, z, arenaSizeX, arenaSizeY, arenaSizeZ;
        arenaSizeX = reader.readInt();
        arenaSizeY = reader.readInt();
    ...
    private static CurrentArenaData readTowerG3(final XDataReader reader,
            final int ver, final ProgressTracker pt) throws IOException {
        int y, x, z, arenaSizeX, arenaSizeY, arenaSizeZ;
        arenaSizeX = reader.readInt();
        arenaSizeY = reader.readInt();
    ...
    private static CurrentArenaData readTowerG4(final XDataReader reader,
            final int ver, final ProgressTracker pt) throws IOException {
        int y, x, z, arenaSizeX, arenaSizeY, arenaSizeZ;
        arenaSizeX = reader.readInt();
        arenaSizeY = reader.readInt();
    ...
    private static CurrentArenaData readTowerG5(final XDataReader reader,
            final int ver, final ProgressTracker pt) throws IOException {
        ...
        for (x = 0; x < arenaSizeX; x++) {
            for (y = 0; y < arenaSizeY; y++) {
                for (z = 0; z < arenaSizeZ; z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        final AbstractArenaObject obj = LTRemix.getApplication()
                                .getObjects().readArenaObjectG5(reader, ver);
                        lt.setCellEra(obj, y, x, z, w,
                                EraConstants.ERA_PRESENT);
                    }
                }
            }
    ...
    private static CurrentArenaData readTowerG6(final XDataReader reader,
            final int ver, final ProgressTracker pt) throws IOException {
        int y, x, z, w, arenaSizeX, arenaSizeY, arenaSizeZ;
        arenaSizeX = reader.readInt();
        arenaSizeY = reader.readInt();
        ...
        for (x = 0; x < arenaSizeX; x++) {
            for (y = 0; y < arenaSizeY; y++) {
                for (z = 0; z < arenaSizeZ; z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        final AbstractArenaObject obj = LTRemix.getApplication()
                                .getObjects().readArenaObjectG6(reader, ver);
                        lt.setCellEra(obj, y, x, z, w,
                                EraConstants.ERA_PRESENT);
                    }
                }
        ...
        for (x = 0; x < CurrentArenaData.PLAYER_DIMS; x++) {
            for (y = 0; y < CurrentArenaData.NUM_PLAYERS; y++) {
                lt.startData.setCell(reader.readInt(), x, y,
                        EraConstants.ERA_PRESENT);
            }
        }
        if (pt != null) {
    ...
    private static CurrentArenaData readTowerG7(final XDataReader reader,
            final int ver, final ProgressTracker pt) throws IOException {
        ...
        for (e = 0; e < EraConstants.MAX_ERAS; e++) {
            for (x = 0; x < arenaSizeX; x++) {
                for (y = 0; y < arenaSizeY; y++) {
                    for (z = 0; z < arenaSizeZ; z++) {
                        for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                            final AbstractArenaObject obj = LTRemix
    ...
    @Override
    public void writeSavedState(final XDataWriter writer,
            final ProgressTracker pt) throws IOException {
        int y, x, z, w;
        writer.writeInt(this.getColumns());
        writer.writeInt(this.getRows());
        writer.writeInt(this.getFloors());
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        ((AbstractArenaObject) this.pastSavedTowerState
                                .getCell(y, x, z, w)).writeArenaObject(writer);
                        ((AbstractArenaObject) this.presentSavedTowerState
                                .getCell(y, x, z, w)).writeArenaObject(writer);
                        ((AbstractArenaObject) this.futureSavedTowerState
                                .getCell(y, x, z, w)).writeArenaObject(writer);
                    }
                }
            }
    ...
    private void readSavedStateG2(final XDataReader reader,
        ...
        this.presentSavedTowerState = new LowLevelArenaDataStore(saveSizeY,
                saveSizeX, saveSizeZ, ArenaConstants.NUM_LAYERS);
        for (x = 0; x < saveSizeX; x++) {
            for (y = 0; y < saveSizeY; y++) {
                for (z = 0; z < saveSizeZ; z++) {
                    this.presentSavedTowerState.setCell(
                            LTRemix.getApplication().getObjects()
        ...
        // Backwards compatibility
        try {
            this.pastSavedTowerState = (LowLevelArenaDataStore) this.presentSavedTowerState
                    .clone();
            this.futureSavedTowerState = (LowLevelArenaDataStore) this.presentSavedTowerState
                    .clone();
        } catch (final CloneNotSupportedException cnse) {
            LTRemix.logError(cnse);
        }
    ...
    private void readSavedStateG5(final XDataReader reader,
        ...
        for (x = 0; x < saveSizeX; x++) {
            for (y = 0; y < saveSizeY; y++) {
                for (z = 0; z < saveSizeZ; z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        this.presentSavedTowerState.setCell(LTRemix
                                .getApplication().getObjects()
                                .readArenaObjectG5(reader, formatVersion), y, x,
                                z, w);
                    }
                }
            }
            ...
            for (y = 0; y < saveSizeY; y++) {
                for (z = 0; z < saveSizeZ; z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        this.presentSavedTowerState.setCell(LTRemix
                                .getApplication().getObjects()
                                .readArenaObjectG6(reader, formatVersion), y, x,
                                z, w);
                    }
                }
            }
    ...
    private void readSavedStateG7(final XDataReader reader,
            final int formatVersion, final ProgressTracker pt)
            throws IOException {
        int y, x, z, w, saveSizeX, saveSizeY, saveSizeZ;
        saveSizeX = reader.readInt();
        ...
        for (x = 0; x < saveSizeX; x++) {
            for (y = 0; y < saveSizeY; y++) {
                for (z = 0; z < saveSizeZ; z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        this.pastSavedTowerState.setCell(LTRemix
                                .getApplication().getObjects()
                                .readArenaObjectG7(reader, formatVersion), y, x,
                                z, w);
                        this.presentSavedTowerState.setCell(LTRemix
                                .getApplication().getObjects()
                                .readArenaObjectG7(reader, formatVersion), y, x,
                                z, w);
                        this.futureSavedTowerState.setCell(LTRemix
                                .getApplication().getObjects()
                                .readArenaObjectG7(reader, formatVersion), y, x,
                                z, w);
                    }
    ...
    public void updateUndoHistory(final HistoryStatus whatWas) {
        try {
            this.iue.updateUndoHistory(
                    (LowLevelArenaDataStore) this.pastEraData.clone(),
                    (LowLevelArenaDataStore) this.presentEraData.clone(),
                    (LowLevelArenaDataStore) this.futureEraData.clone(),
                    whatWas);
        } catch (final CloneNotSupportedException cnse) {
            LTRemix.logError(cnse);
        }
    ...
    private class ImageUndoEngine {
        ...
        public void updateUndoHistory(final LowLevelArenaDataStore newPastImage,
                final LowLevelArenaDataStore newPresentImage,
                final LowLevelArenaDataStore newFutureImage,
                final HistoryStatus newWhatWas) {
            this.undoHistory.push(newPastImage, newPresentImage, newFutureImage,
                    newWhatWas);
        }
        ...
        public void updateRedoHistory(final LowLevelArenaDataStore newPastImage,
                ...
                final LowLevelArenaDataStore newFutureImage,
                final HistoryStatus newWhatWas) {
            this.redoHistory.push(newPastImage, newPresentImage, newFutureImage,
                    newWhatWas);
        }
        ...
        private class HistoryEntry {
            ...
            HistoryEntry(final LowLevelArenaDataStore pastI,
                    final LowLevelArenaDataStore presentI,
                    final LowLevelArenaDataStore futureI,
        ...
        private class HistoryStack {
            ...
            public void push(final LowLevelArenaDataStore pastI,
                    final LowLevelArenaDataStore presentI,
                    final LowLevelArenaDataStore futureI,
                    final HistoryStatus hww) {
                final HistoryEntry newEntry = new HistoryEntry(pastI, presentI,
                        futureI, hww);
