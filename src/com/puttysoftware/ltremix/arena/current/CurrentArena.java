...
public class CurrentArena extends AbstractArena {
    ...
    public static int getProgressStages() {
        return CurrentArena.PROGRESS_STAGES
                + CurrentArenaData.getProgressStages();
    }
    ...
    protected boolean removeActiveLevel() {
        if (this.levelCount > 1) {
            if (this.activeLevel >= 0 && this.activeLevel <= this.levelCount) {
                ...
                final boolean res = this.getLevelFile(this.activeLevel)
                        .delete();
                if (!res) {
                    return false;
                }
    ...
    @Override
    public int[] tunnelScan(final int x, final int y, final int z,
    ...
    @Override
    public void setPlayerRow(final int newStartRow, final int playerNum,
            final int era) {
        this.arenaData.setPlayerRow(newStartRow, playerNum, era);
    }

    @Override
    public void setPlayerColumn(final int newStartColumn, final int playerNum,
            final int era) {
        this.arenaData.setPlayerColumn(newStartColumn, playerNum, era);
    }

    @Override
    public void setPlayerFloor(final int newStartFloor, final int playerNum,
            final int era) {
    ...
    public CurrentArena readArena(final ProgressTracker pt) throws IOException {
        ...
        try (XDataReader metaReader = DataIOFactory.createTagReader(m.basePath
            ...
            if (FormatConstants.isFormatVersionValidGeneration4(version)
                    ...
                    || FormatConstants
                            .isFormatVersionValidGeneration7(version)) {
    ...
    private void writeArenaLevelProgress(final XDataWriter writer,
