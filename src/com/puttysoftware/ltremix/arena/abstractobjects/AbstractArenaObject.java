...
public abstract class AbstractArenaObject extends CloneableObject {
	...
	if (this.isSolid()) {
		...
		final AbstractArenaObject adj = LTRemix.getApplication().getArenaManager().getArena()
			.getCell(locX - dirX, locY - dirY, locZ, this.getPrimaryLayer());
	...
	return StringLoader.loadImageString(StringConstants.OBJECT_STRINGS_FILE, this.getStringBaseID() * 3 + 0);
    }
    ...
    public final AbstractArenaObject readArenaObjectG7(final XDataReader reader, final String ident, final int ver)
	    throws IOException {
	if (ident.equals(this.getIdentifier())) {
	    final int cc = this.getCustomFormat();
	    if (cc == AbstractArenaObject.CUSTOM_FORMAT_MANUAL_OVERRIDE) {
     ...
     * @throws IOException
     */
    protected AbstractArenaObject readArenaObjectHookG7(final XDataReader reader, final int formatVersion)
	    throws IOException {
	// Dummy implementation, subclasses can override
	return this;
    }
