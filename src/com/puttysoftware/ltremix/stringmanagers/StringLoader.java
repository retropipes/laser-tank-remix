...
public class StringLoader {
    ...
    public static String loadImageString(final int fileID, final int strID) {
        return StringLoader.IMAGE_STRING_CACHE.get(fileID)
                .get(Integer.valueOf(strID));
    }
    ...
    private static void cacheImageStringFile(final int fileID) {
        ...
        try (final InputStream is = StringLoader.LOAD_CLASS.getResourceAsStream(
            ...
            while (line != null) {
                ...
                if (line != null) {
                    ...
                    if (StringLoader.IMAGE_STRING_CACHE.size() <= fileID
                            || StringLoader.IMAGE_STRING_CACHE
                                    .get(fileID) == null) {
                        // Entry for string file doesn't exist, so create it
                        for (int x = 0; x <= fileID; x++) {
                    ...
                    StringLoader.IMAGE_STRING_CACHE.get(fileID).put(
                            Integer.valueOf(Integer.parseInt(splitLine[0])),
                            splitLine[1]);
                }
            }
            ...
            CommonDialogs.showErrorDialog(
                    "Something has gone horribly wrong trying to load the image string data!",
                    "FATAL ERROR");
            LTRemix.logErrorDirectly(ioe);
        }
    }
